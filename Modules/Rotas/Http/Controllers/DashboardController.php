...
class DashboardController extends Controller
{
    ...
    {
        if (Auth::check()) {
            if (Auth::user()->can('rotas dashboard manage')) {
                ...
                if (!in_array(Auth::user()->type, Auth::user()->not_emp_type)) {
                    ...
                    $employee = Employee::where('user_id', $userId)->get()->toArray();
                }
                $employee_id = '';
                $employee_data = [];
                if (!empty($employee)) {
                ...
                foreach ($rotas_date as $date) {
                    ...
                    if (!empty($rotas)) {
                        foreach ($rotas as $rota) {
                            $profile_data = Employee::whereRaw('employee_id = ' . $rota['user_id'] . ' ')->first();
                            $color        = '#8492a6';
                            $nameee       = '-';
                            $location = Designation::where('id', $rota['designation_id'])->first();
        ...
        foreach ($rotas_date as $date) {
            ...
            if (!empty($rotas)) {
                ...
                foreach ($rotas as $rota) {
                    ...
                    $location = Designation::whereRaw('id = ' . $rota['designation_id'] . ' ')->first();
    ...
    public function dayView()
    {
        ...
        $employee = Employee::where('created_by', $created_by)->get()->toArray();
        ...
        $locations       = [];
        if (Auth::user()->type == 'company') {
            $locations = Designation::where('created_by', creatorId())->where('workspace', getActiveWorkSpace())->get();
        }

        if (!in_array(Auth::user()->type, Auth::user()->not_emp_type)) {
            $locations_id = Employee::where('user_id', $userId)->first();
            if (!empty($locations_id->branch_id)) {
                $locations = Designation::where('id', $locations_id->designation_id . ')')->get();
            }
        }
    ...
    public function dayview_filter(Request $request)
    {
        ...
        if (!empty($emp_name)) {
            ...
            if (Auth::user()->type == 'company') {
                $employee = Employee::where('created_by', $created_by)->orwhere('id', $created_by)->pluck('id')->toArray();
            } else {

                $employee = Employee::where('user_id', $userId)->pluck('id')->toArray();
            }
        }
    ...
    public function userView()
    {
        ...
        if (Auth::user()->type == 'company') {
            $employee_id = Employee::where('created_by', $created_by)->orwhere('id', $created_by)->pluck('id')->toArray();
        } else {
            $employee_id = Employee::where('user_id', $userId)->pluck('id')->toArray();
        }

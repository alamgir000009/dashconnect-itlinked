...
class RotaController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (Auth::user()->can('rota manage')) {
            ...
            if (!in_array(Auth::user()->type, Auth::user()->not_emp_type)) {
                $emp_only_see_own_rota = (!empty(company_setting('emp_only_see_own_rota')) && company_setting('emp_only_see_own_rota') == 1) ? $user->id : 0;
                $employee_permission['emp_only_see_own_rota'] = (!empty(company_setting('emp_only_see_own_rota'))) ? $user : 0;
    ...
    public function un_publish_week(Request $request)
    {
        ...
        if (Auth::user()->can('rota unpublish-week')) {
    ...
    {
        if (Auth::user()->can('rota publish-week')) {
    ...
    {
        if (Auth::user()->can('rota copy-week-shift')) {
    ...
    {
        if (Auth::user()->can('rota shift-copy')) {
    ...
    {
        if (Auth::user()->can('rota hide/show day off')) {
    ...
    {
        if (Auth::user()->can('rota hide/show leave')) {
    ...
    {
        if (Auth::user()->can('rota hide/show availability')) {
    ...
    public function send_email_rotas(Request $request)
    {
        ...
        if (Auth::user()->can('rota send mail')) {
            if (!empty(company_setting('Rotas')) && company_setting('Rotas') == true) {
    ...
    {
        if (Auth::user()->can('rota clear week')) {
    ...
    public function printrotasInvoice(Request $request)
    {
        if (Auth::user()->can('rota print')) {
            if (!empty($request->user)) {
                ...
                $company_date_format = (!empty(company_setting('company_date_format'))) ? company_setting('company_date_format') : 'd M Y';
    ...
    public function setting(Request $request)
    {
        ...
        \Settings::context($userContext)->set('emp_show_rotas_price', $request->emp_show_rotas_price);
        \Settings::context($userContext)->set('emp_show_avatars_on_rota', $request->emp_show_avatars_on_rota);
        \Settings::context($userContext)->set('emp_hide_rotas_hour', $request->emp_hide_rotas_hour);
        \Settings::context($userContext)->set('include_unpublished_shifts', $request->include_unpublished_shifts);
        \Settings::context($userContext)->set('emp_only_see_own_rota', $request->emp_only_see_own_rota);

        \Settings::context($userContext)->set('company_week_start', $request->company_week_start);
        \Settings::context($userContext)->set('break_paid', $request->break_paid);
        \Settings::context($userContext)->set('see_note', $request->see_note);
        \Settings::context($userContext)->set('employees_can_set_availability', $request->employees_can_set_availability);
        \Settings::context($userContext)->set('leave_start_month', $request->leave_start_month);
        return redirect()->back()->with('success', __('Rota Setting successfully updated.'));
    }
    ...
    public function companyworkschedule(Request $request)
    {
        $company_setting = User::find(Auth::user()->id);
        $userContext = new Context(['user_id' => \Auth::user()->id, 'workspace_id' => getActiveWorkSpace()]);
        \Settings::context($userContext)->set('WorkSchedule', $request->work_schedule);

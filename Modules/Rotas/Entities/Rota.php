...
class Rota extends Model
{
    ...
    public static function getWorkSchedule($id = 0, $week = 0, $designation_id = 0)
    {
        ...
        $emp_hide_rotas_hour = (!empty(company_setting('emp_hide_rotas_hour'))) ? company_setting('emp_hide_rotas_hour') : 0;

        $emp_show_rotas_price = 0;
        if (Auth::user()->type != 'company') {
            $emp_show_rotas_price = (!empty(company_setting('emp_show_rotas_price')) && company_setting('emp_show_rotas_price') == 1) ? company_setting('emp_show_rotas_price') : 0;
        } else {
        ...
        if ($id != 0) {
            ...
            $view_setting_array = [];
            if (!empty(company_setting('see_note'))) {
                $view_setting_array = company_setting('see_note');
            }
            if (!empty($user->avatar) && company_setting('emp_show_avatars_on_rota') == 1) {
            ...
            if (!empty(company_setting('see_note')) && company_setting('see_note') == 'none' && !in_array(Auth::user()->type, Auth::user()->not_emp_type)) {
                $class1 = 'd-none ';
            }
            if (!empty(company_setting('see_note')) && company_setting('see_note') == 'self' && !in_array(Auth::user()->type, Auth::user()->not_emp_type) && $login_employee->id != $id) {
                $class1 = 'd-none ';
            }
            if (!empty(company_setting('see_note')) && company_setting('see_note') == 'all' && !in_array(Auth::user()->type, Auth::user()->not_emp_type)) {
                $class1 = '';
            }
        ...
        if ($id != 0) {
            ...
            $currency_symbol = (!empty(company_setting('company_currency_symbol'))) ? company_setting('company_currency_symbol') : '$';
